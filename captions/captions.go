...
func hormoziCaptionsFrom(captions Captions) string {
  ...
  [V4+ Styles]
  Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
  Style: Default,The Bold Font,24,&H00FFFFFF,&H00FFFF00,&H00000000,&H64000000,-1,0,0,0,100,100,0,0,1,1,0,2,10,10,10,1
	...
	for i, event := range captions.Events {
		...
		for j, s := range event.Segs {
			...
			for k, seg := range event.Segs {
				if k == j {
					dialogue += " " + "{\\c&H00FFFF&}" + strings.TrimSpace(seg.UTF8) + "{\\c&HFFFFFF&}"
				} else {
